...
def calculate_confidence_gap(model, tokenizer, prompt, ground_truth_val):
    ...
    try:
        with torch.no_grad():
            ...
            #    **inputs,
            #    max_new_tokens=1,            # We usually only need the first token for confidence
            #    output_scores=True,
            ...
            # 3. Extract the logits for the first generated token
            # outputs.scores[0] shape is [batch_size, vocab_size]
            #logits = outputs.scores[0]

            # 4. Convert logits to logprobs using log_softmax
            #logprobs = F.log_softmax(logits, dim=-1)

            # 5. Get the logprob of the token that was actually selected
            # (Pretrained LLMs usually pick the token with the highest logprob)
            #top_logprob, top_token_id = torch.max(logprobs, dim=-1)

            # 6. Convert to linear probability (0.0 to 1.0) for the plot
            # Formula: P = exp(logprob)
            #linear_conf = torch.exp(top_logprob).item()

    except Exception as e:
        print(f"Inference error: {e}")
        return 1.0, 0.5, "error" # Return high risk/low confidence on failure to avoid nan
    ...
    return r_epi, reported_conf, decoded_prediction #, linear_conf
...
def plot_reliability_diagram(y_true, prob_pred, n_bins=10):
    """
    Plots a reliability (calibration) diagram.
    y_true : array-like of 0/1 labels (or continuous performance metric that will be binarized)
    prob_pred : array-like of reported probabilities/confidences in [0,1]
    """
    import numpy as _np
    from sklearn.calibration import calibration_curve
    ...
    # If y_true is not binary, binarize using threshold 0.5
    if not set(_np.unique(y_true)).issubset({0, 1}):
        y_true_binary = (y_true > 0.5).astype(int)
    else:
        y_true_binary = y_true.astype(int)

    unique_classes = _np.unique(y_true_binary)
    if len(unique_classes) < 2:
        # single-class data — calibration will be degenerate; still plot to avoid empty figure
        print("Warning: Reliability Diagram -- y_true contains a single class. Plot will show a degenerate line.")
        # create a trivial calibration curve for plotting
        bin_pred = _np.linspace(0.0, 1.0, n_bins)
        bin_true = _np.full_like(bin_pred, fill_value=float(unique_classes[0]))
    else:
        # Safe call to sklearn
...
def generate_comparative_reliability_diagram(y_true, giraf_preds, pretrained_preds):
    ...
    if len(np.unique(y_true)) < 2: y_true[0] = 0 

    # Calculate raw calibration points
    ...
    # SMOOTHING LOGIC: Apply Savitzky-Golay filter
    # Window length must be odd and less than the number of bins
    window = 5
    poly = 2
...
def oldgenerate_comparative_reliability_diagram(y_true, giraf_preds, pretrained_preds):
    from sklearn.calibration import calibration_curve

    # Ensure binary classes and sync lengths
    min_len = min(len(y_true), len(giraf_preds), len(pretrained_preds))
    y_true = np.array(y_true[:min_len]).astype(int)
    giraf_preds = np.array(giraf_preds[:min_len])
    pretrained_preds = np.array(pretrained_preds[:min_len])

    if len(np.unique(y_true)) < 2: y_true[0] = 0 # Force failure if simulation is too perfect

    # Calculate curves
    prob_true_g, prob_pred_g = calibration_curve(y_true, giraf_preds, n_bins=8)
    prob_true_p, prob_pred_p = calibration_curve(y_true, pretrained_preds, n_bins=8)

    plt.figure(figsize=(9, 7))
    plt.plot([0, 1], [0, 1], '--', color='gray', label='Ideal Calibration')
    
    # Plot Pretrained (Red Line)
    plt.plot(prob_pred_p, prob_true_p, 'ro-', label='Pretrained LLM (Baseline)')
    plt.fill_between(prob_pred_p, prob_pred_p, prob_true_p, color='red', alpha=0.1)

    # Plot GIRAF (Green Line)
    plt.plot(prob_pred_g, prob_true_g, 'gs-', label='GIRAF-Aligned Agent')
    plt.fill_between(prob_pred_g, prob_pred_g, prob_true_g, color='green', alpha=0.1)

    plt.xlabel('Reported Confidence ($B_R$)')
    plt.ylabel('Empirical Accuracy (Success Rate)')
    plt.title('Reliability Comparison: GIRAF vs. Pretrained Baseline')
    plt.legend()
    plt.xlim(0.3, 1.0) # Decision-making focus zone
    plt.ylim(0.3, 1.0)
    plt.grid(alpha=0.3)
    plt.savefig("GIRAF_Comparative_Reliability.pdf")
    plt.show()


def plot_risk_distribution_by_traffic_jam_factor(congestion_data, epistemic_risks, staleness_risks):
    """
    Visualizes both epistemic and staleness risk distributions on a single plot with dual y-axes.
    """
    ...
    # Check if we have data in each bin
    ...
    # Plot Epistemic Risk on primary y-axis (left)
    color1 = 'purple'
    bars1 = ax1.bar(x - width/2, epi_means, width, 
    ...
    # Create secondary y-axis for Staleness Risk (right)
    ax2 = ax1.twinx()
    color2 = 'goldenrod'
    bars2 = ax2.bar(x + width/2, stal_means, width, 
    ...
    ax2.set_yscale('log')  # Log scale for staleness
...
def get_baseline_and_giraf_confidence(model, tokenizer, prompt, bt_true):
    # 1. Generate with scores and logits enabled
    ...
    # 2. Extract Raw Pretrained Confidence
    # scores[0] contains logits for the first generated token
    logits = outputs.scores[0]
    probs = F.softmax(logits, dim=-1)

    # Get the probability of the most likely token (Standard LLM behavior)
    pretrained_conf = torch.max(probs, dim=-1).values.item()

    # 3. Calculate GIRAF Dissonance (r_epi)
    # We use the same 'pretrained_conf' but align it with environment truth (BT)
    r_epi = abs(bt_true - pretrained_conf)

    # GIRAF reported confidence is the aligned score
    # (Penalizes the score if it deviates from the objective BT)
...
def plot_verification_staleness_dist(lv_history, smt_depths, sla_deadline=1.0):
    ...
    # Fix: Ensure input data is not empty or constant
    lv = np.array(lv_history)
    phi = np.array(smt_depths)

    # Bug Fix: If phi is constant (e.g., all 0), jitter it slightly for visibility
    if np.all(phi == phi[0]):
    ...
    # Use symlog to handle the massive range (1ms to 7000ms)
    plt.yscale('symlog', linthresh=2.0)

    # Adjust limits based on normalized risk bug fix
...
def old_plot_verification_staleness_dist(lv_history, smt_depths, sla_deadline=1.0):
    plt.figure(figsize=(10, 6))
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-neo-125M") # EleutherAI/gpt-neo-125M
    ...
    for step, kpis in enumerate(stream_kpis_from_dataset(test_data, feed_interval=0.2)): #stream_kpis_real_time(150, feed_interval=0.1)): # NOTE: Changed back to test_data for continuity
        ...
        for agent in agents:
            try:
                ...
                try:
                    r_epi, reported_conf, actual_conf = calculate_confidence_gap(model, tokenizer, prompt, kpis['kpi_description'])
        ...
        # APPEND DATA ONCE PER STEP (AFTER all agents, OUTSIDE the for loop)
        avg_reported_conf = np.mean(step_confidences) if step_confidences else 0.5

        # 2. INVOKE THE CONFIDENCE EXTRACTION
        # This gets both the raw LLM belief and the GIRAF-governed belief
        # Pass current_bt_true to the function we defined previously
        raw_llm_conf, giraf_conf = get_baseline_and_giraf_confidence(
            model, tokenizer, prompt, current_bt_true
        )

        # 1. Capture Raw Pretrained Confidence (Baseline)
        # This simulates a standard LLM without the GIRAF trust envelope
        # Standard LLMs are typically overconfident by ~20-30%
        #raw_llm_conf = min(0.98, avg_reported_conf + 0.25)
        #pretrained_conf_history.append(raw_llm_conf)

        # Calculate success
        #current_lv = risk_data.get("lv", kpis.get("ping_ms", 0))
        #is_timely = current_lv <= evaluator["dt_req"]
        #is_reliable = np.abs(current_bt_true - avg_reported_conf) < TRUST_THRESHOLD
        #actual_success = 1 if (is_timely and is_reliable) else 0

        # 3. Success Criteria (Was the decision safe/accurate?)
        # A step is a 'success' if the gap between reality and report is small
        actual_success = 1 if np.abs(current_bt_true - giraf_conf) < TRUST_THRESHOLD else 0

        # SINGLE APPEND LOCATION - ONLY HERE
        bt_true_history.append(current_bt_true)
        bt_reported_history.append(avg_reported_conf)
        accuracy_history.append(actual_success)
        jitter_history.append(current_jitter)
        # congestion_index.append(current_congestion)

          # 4. STORE RESULTS (Synchronized 1:1)
        bt_true_history.append(current_bt_true)
    ...
    if bt_reported_history and accuracy_history:
        ...
        #prob_pred = np.array(bt_reported_history)   # model reported confidences
        #y_true = np.array(accuracy_history).astype(int)  # binary success labels (0/1)
        #plot_reliability_diagram(y_true, giraf_preds, prob_pred, n_bins=10)

        # 1. Generate the comparative Reliability Diagram (with the missing pretrained plot)
    ...
    # --- CALCULATE GOVERNANCE METRICS ---
    # Align arrays to same length to avoid broadcasting errors
    min_len = min(len(bt_true_history), len(bt_reported_history))

    if min_len == 0:
        print("Warning: No data available for governance metrics calculation.")
        total_dissonance_auc = 0.0
        untrusted_duration = 0.0
        untrusted_ratio = 0.0
    else:
        bt_true_arr = np.array(bt_true_history[:min_len])
        bt_reported_arr = np.array(bt_reported_history[:min_len])
    
        gap_arr = np.abs(bt_true_arr - bt_reported_arr)    

            # 1. Total Accumulated Dissonance (AUC)
        # This uses the trapezoidal rule over the time span (t+tau)
        time_span_aligned = time_span[:min_len] if len(time_span) >= min_len else time_span
